...
class ImageMap:
    ...
    def update(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                self.click_and_change_pos(*event.pos)
    ...
    def click_and_change_pos(self, x, y):
        dy = 255 - y
        dx = x - 300
        self.lon = self.lon + dx * CONVERT_COORDS * math.pow(2, 15 - self.zoom)
        self.lat = self.lat + dy * CONVERT_COORDS * math.cos(math.radians(self.lat)) * math.pow(2, 15 - self.zoom)
